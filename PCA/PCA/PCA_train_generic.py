...
"""
 ...
 Note : The path to directory should not contain underscore or dots till the folder. 
	The code works for irrespetive of any directory structure. But as far as possible try to avoid underscores and dots in the names of the directory.	
...
#Function to return the split file names
...
# This piece of code is made as function with the name : return_split_file_names(images).
...
# Uncomment this part to see the split file names
...
# If the directory structure is different, some change to be done to the path names of the files. 
# Every database has more then one class, this is obvious, coz if there are more then one class only then face recogniton on that databse makes some sense
...
#Checking if the classes are partitioned properly.
	...
	if(no_of_classes<=1):
		...
		temp_index_if_present=temp_str_for_checking_if_underscore_is_present.find('_')
		print temp_index_if_present
		if(temp_index_if_present>=0):
			flag_for_changing_file_name=1
		else:
			flag_for_changing_file_name=0
		for i in range(no_of_images):
...
#Uncomment following to know 
	
		#print split_image_names

# Uncomment following lines to see the modified file names
...
# assigning back to images very important thing to do.
		
	#	images=images_name_modified 
...
#Uncomment follwing  to print signature of the trained images
